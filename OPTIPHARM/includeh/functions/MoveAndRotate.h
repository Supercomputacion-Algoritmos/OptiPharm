...
public:
	...
	void static PreciseMolToNewPosition(double *atomsMolXYZ, unsigned int size, double deltaX, double deltaY,
					double deltaZ);
	/**
	 * Dada una molecula, dos puntos en el espacio y un angulo, se obtiene la molecula rotada en un sentido.
	 */
	...
	//void static RotateAtomAccordingAngleXYZ(Atom* atomRotate, double theta, double x, double y, double z);
	//void static RotateMolAccordingAngleXYZ(const Molecule* mol, double theta, double x, double y,double z, Molecule* molRotate);
	/**
