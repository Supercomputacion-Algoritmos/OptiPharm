...
public:
	...
	virtual ~NDimMolElement() { if( x != NULL ) delete[] x; };
	...
	virtual SearchSpElement* generateMutationNew(short, SearchSpElement* s=NULL);
	...
	virtual void	SetX( double* y){for (int k=0; k<dim;k++) x[k]= (y[k] -INI.Lowb(k)) / (INI.Upb(k) - INI.Lowb(k));
	}
};
