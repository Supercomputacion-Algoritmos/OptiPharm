...
void Molecule::setTanimoto(double* atomsXYZ,  unsigned int size,double *weight, double*radius, bool sameVDWR) {
...
void Molecule::ObjectToArray(double* atoms, double* radius, double* weights,
	...
	for (int i = 0; i < numberOfAtoms; i++) {
		iterador = i * 3;
		weights[i] = VolumeOverlap::calculateWeightWEGA(atoms[iterador],
				atoms[iterador + 1], atoms[iterador + 2], numberOfAtoms, atoms,
				radius, i, sameVanderWaalsRadius);
		//cout << "Peso de "<<i<<"["<<atoms[iterador]<<", "<<atoms[iterador + 1]<<", "<<atoms[iterador + 2]<<"]: "<<weights[i]<<"\n";
...
void Molecule::calculateWeightWEGAAtoms(double * atoms, unsigned int nAtoms,
	...
	for (int i = 0; i < nAtoms; i++) {
		iterador = i * 3;
		weights[i] = VolumeOverlap::calculateWeightWEGA(atoms[iterador],
				atoms[iterador + 1], atoms[iterador + 2], nAtoms, atoms, radius,
				i, sameVanderWaalsRadius);
	}
