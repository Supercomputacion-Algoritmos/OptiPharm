...
void MoveAndRotate::PreciseMolToNewPosition(double *atomsMolXYZ, unsigned int size,
		double deltaX, double deltaY, double deltaZ) {
	int iterador = 0;
	for (unsigned i = 0; i < size; i++) {
		iterador = i * 3;
...
inline double fastCos(double x, int len = 4) { //USE BETWEEN -pi, pi
	...
	double fac[] = { 0.5, 0.041666667, 1.388888889e-3, 2.48015873e-5,
			2.755731922e-7, 2.087675699e-9 }; //2!, 4!, 6!, 8!, 10!, 12!...
...
inline double fastSin(double x, int len = 4) { //USE BETWEEN -pi, pi
	...
	double fac[] = { 0.166666667, 8.333333333e-3, 1.984126984e-4,
			2.755731922e-6, 2.505210839e-8, 1.605904384e-10 }; //3!, 5!, 7!, 9!, 11!, 13!...
...
void MoveAndRotate::PreciseRotateMolAccording1Axis(const double* molAtomsXYZ,
		unsigned int nAtoms, double theta, double axisAx, double axisAy,
		double axisAz, double axisBx, double axisBy, double axisBz,
		double* molAtomsRotated) {
...
/*void MoveAndRotate::RotateAtomAccordingAngleXYZ(Atom* atomRotate, double theta, double x, double y, double z) {
...
/*void MoveAndRotate::RotateMolAccordingAngleXYZ(const Molecule* mol, double theta, double x, double y,double z, Molecule* molRotate) {
 *molRotate = *mol;
...
/**
 * Dados dos puntos en el espacio y un angulo se devuelve un atomo rotado en una direccion y otro en la otra
 */
