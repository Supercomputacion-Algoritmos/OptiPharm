...
double VolumeOverlap::calculateVolumeOverlapHardSphere(Molecule* fixedMol,
	...
	for (unsigned i = 0; i < fixedMol->atoms.size(); i++) {
		xmin = min(xmin,
				fixedMol->atoms[i].x - fixedMol->atoms[i].radiusVanDerWaals);
		ymin = min(ymin,
				fixedMol->atoms[i].y - fixedMol->atoms[i].radiusVanDerWaals);
		zmin = min(zmin,
				fixedMol->atoms[i].z - fixedMol->atoms[i].radiusVanDerWaals);
	...
	for (unsigned i = 0; i < variableMol->atoms.size(); i++) {
		xmin = min(xmin,
				variableMol->atoms[i].x
						- variableMol->atoms[i].radiusVanDerWaals);
		ymin = min(ymin,
				variableMol->atoms[i].y
						- variableMol->atoms[i].radiusVanDerWaals);
		zmin = min(zmin,
				variableMol->atoms[i].z
						- variableMol->atoms[i].radiusVanDerWaals);
	...
	for (double xcor = xmin; xcor < xmax; xcor += res) {
		for (double ycor = ymin; ycor < ymax; ycor += res) {
			for (double zcor = zmin; zcor < zmax; zcor += res) {
				...
				for (unsigned i = 0;
						i < fixedMol->atoms.size() && !refAtomFound; i++) {
				...
				for (unsigned i = 0;
						i < variableMol->atoms.size() && !fitAtomFound; i++) {
					fitAtomFound =
							(variableMol->atoms[i].radiusVanDerWaals
									* variableMol->atoms[i].radiusVanDerWaals
									>= distsqr(xcor, ycor, zcor,
											variableMol->atoms[i]));
				}
				if (fitAtomFound)
					count++;
...
double VolumeOverlap::calculateOverlapVolumeGaussAnalyticWEGA(Molecule* fixed,
		Molecule* variable) {
...
double VolumeOverlap::calculateOverlapVolumeGaussAnalyticWeight(Molecule* fixed,
		Molecule* variable) {
	const double partialalpha = 2.41798793102;
	const double pi = 3.14159265358;
	double overlap = 0.0;
...
double VolumeOverlap::calculateOverlapVolumeGaussAnalyticWeightWEGA(
	...
	for (unsigned i = 0; i < fixed->atoms.size(); i++) {
		for (unsigned j = 0; j < variable->atoms.size(); j++) {
			//Calculo Vij con el metodo porque he visto que se calculaba de la misma forma que en el caso del peso.
			double Vij = calculateOverlapVolumeAtomsGaussAnalytic(
					&(fixed->atoms[i]), &(variable->atoms[j]));

			double WeightVij = calculateWeight(&(fixed->atoms[i]),
					&fixed->atoms, i)
					* calculateWeight(&(variable->atoms[j]), &variable->atoms,
							j) * Vij;
			overlap += WeightVij;
			//overlap += Vij;
...
double VolumeOverlap::overlapWEGA(Molecule* fixed, Molecule* variable,
	...
	for (unsigned i = 0; i < fixed->atoms.size(); i++) {
		for (unsigned j = 0; j < variable->atoms.size(); j++) {
			//Calculo Vij con el metodo porque he visto que se calculaba de la misma forma que en el caso del peso.
			double Vij = calculateOverlapVolumeAtomsWEGA(&(fixed->atoms[i]),
					&(variable->atoms[j]), sameVanDerWaalsRadius);

			double WeightVij = calculateWeightWEGA(&(fixed->atoms[i]),
					&fixed->atoms, i, sameVanDerWaalsRadius)
					* calculateWeightWEGA(&(variable->atoms[j]),
							&variable->atoms, j, sameVanDerWaalsRadius) * Vij;
...
double VolumeOverlap::overlapWEGA(double* fixed, unsigned int fixedSize,
		double *weightFixed, double*radiusFixed, double* variable,
		unsigned int variableSize, double * weightVariable,
		double* radiusVariable, bool sameVanDerWaalsRadius) {
	...
	double overlap = 0.0;
		...
		for (unsigned int i = 0; i < fixedSize; i++) {
			...
			for (unsigned int j = 0; j < variableSize; j++) {
				int iteradorj = j * 3;
				//Calculo Vij con el metodo porque he visto que se calculaba de la misma forma que en el caso del peso.
				double Vij = calculateOverlapVolumeAtomsWEGA(fixed[iteradori],
						fixed[iteradori + 1], fixed[iteradori + 2],
						variable[iteradorj], variable[iteradorj + 1],
						variable[iteradorj + 2], sameVanDerWaalsRadius,
						radiusFixed[i], radiusVariable[j]);
				...
				/*	if (overlap > 1000){
				 ...
				 fixed[iteradori + 2]<<", "<<variable[iteradorj]<<", "<< variable[iteradorj + 1]
	...
	double overlap = 0.0;
		...
		for (unsigned int i = 0; i < fixedSize; i++) {
			...
			for (unsigned int j = 0; j < variableSize; j++) {
				int iteradorj = j * 3;
				//Calculo Vij con el metodo porque he visto que se calculaba de la misma forma que en el caso del peso.
				double Vij = preciseCalculateOverlapVolumeAtomsWEGA(fixed[iteradori],
						fixed[iteradori + 1], fixed[iteradori + 2],
						variable[iteradorj], variable[iteradorj + 1],
						variable[iteradorj + 2], sameVanDerWaalsRadius,
						radiusFixed[i], radiusVariable[j]);
...
double rho(Atom atom, Point3DDouble gc) {
	...
	double alpha = partialalpha
			/ (atom.radiusVanDerWaals * atom.radiusVanDerWaals);
	...
	for (int x = 0; x < grid.extent.x; x++) {
		for (int y = 0; y < grid.extent.y; y++) {
			for (int z = 0; z < grid.extent.z; z++) {
				...
				Point3DDouble gc = gridSpaceToRealSpace(grid.res,
						grid.lowestPoint, x, y, z);
...
// Esto esta copiado de PAPERS y no creo que sirva. Utiliza otra forma de calcular el solapamiento entre dos atomos
/*
	...
	double alphaj = pow(
			3 * p * sqrt(pi)
					/ (4
							* (atomVariable.radiusVanDerWaals
									* atomVariable.radiusVanDerWaals
									* atomVariable.radiusVanDerWaals)), 2 / 3);
...
double VolumeOverlap::calculateOverlapVolumeAtomsWEGA(Atom* atom1, Atom* atom2,
		bool sameVanDerWaalsRadius) {
	...
	if (sameVanDerWaalsRadius) {
		...
		double alphai = 2.417972471923026
				/ (atomFixed.radiusVanDerWaals * atomFixed.radiusVanDerWaals);
		double alphaj = 2.417972471923026
				/ (atomVariable.radiusVanDerWaals
						* atomVariable.radiusVanDerWaals);
		/*double alphai = pow(
		 3 * p * sqrt(pi)
...
inline double VolumeOverlap::calculateOverlapVolumeAtomsWEGA(double atom1X,
...
inline double VolumeOverlap::preciseCalculateOverlapVolumeAtomsWEGA(double atom1X,
		double atom1Y, double atom1Z, double atom2X, double atom2Y,
		double atom2Z, bool sameVanDerWaalsRadius, double radiusFixed,
		double radiusVariable) {
...
double VolumeOverlap::calculateWeightWEGA(double atomX, double atomY,
		double atomZ, double sizeAtoms, double*atoms, double *radius, int iAtom,
		bool sameVanDerWaalsRadius) {
	...
	for (unsigned i = 0; i < sizeAtoms; i++) {
		...
		vij += calculateOverlapVolumeAtomsWEGA(atomX, atomY, atomZ,
				...
				sameVanDerWaalsRadius, radius[iAtom], radius[i]);
...
void VolumeOverlap::longestShortestAxis(Molecule& mol, double* longest,
	...
	for (unsigned i = 0; i < mol.atoms.size(); i++) {
		double modulo = sqrt(
				mol.atoms[i].x * mol.atoms[i].x
						+ mol.atoms[i].y * mol.atoms[i].y
