...
int Parameters::readArguments(int argc, char** argv, int* indexArray,
	...
	// Leemos los parametros de entrada y obtenemos las configuraciones correspondientes.
...
void Parameters::defineDeltaMovement(Ini* ini, double *minX, double *maxX,
	...
	//cout << minPointMolFixed.x << ", "<< minPointMolFixed.y << ", "<< minPointMolFixed.z << "\n";
	//cout << maxPointMolFixed.x << ", "<< maxPointMolFixed.y << ", "<< maxPointMolFixed.z << "\n";
	...
	//cout << minPointMolVariable.x << ", "<< minPointMolVariable.y << ", "<< minPointMolVariable.z << "\n";
	//cout << maxPointMolVariable.x << ", "<< maxPointMolVariable.y << ", "<< maxPointMolVariable.z << "\n";

	double deltaXMin = 0, deltaXMax = 0, deltaYMin = 0, deltaYMax = 0,
			deltaZMin = 0, deltaZMax = 0;
	//cout << minPointMolFixed.toString()<< "  " <<maxPointMolFixed.toString()<<'\n';
	//cout << minPointMolVariable.toString()<<"   "<<maxPointMolVariable.toString()<<'\n';
...
void Parameters::alignMolecule(Molecule * mol) {
	...
	// We convert the array in a eigen MAtrix. We need to give data, number of rows and number of columns
	...
	for (unsigned i = 0; i < numberOfAtoms; i++) {
		...
		mol->atoms[i].y = m(0, 1);
		mol->atoms[i].z = m(0, 2);
	//	cout << i << ": " << mol->atoms[i].x << ", " << mol->atoms[i].y << ", " << mol->atoms[i].z << "\n";
	}
