...
int main(int argc, char** argv) {
	...
	//0: -c, 1: -q, 2: -d, 3: -o, 4: -h, 5: -w, 6: -N, 7: -M, 8: -L, 9: -R, 10: -pre, 11: -pz, 12: -smol, 13: -fvdwr, 14: -lowL
	int* indexArray = NULL;

	int lenghtIndexArray = 15;
	...
	for (unsigned int imol = 0; imol < db.size(); imol++) {
		...
		ini->setMolQueryToString(
				WriteMolecule::MolToString(ini->getMolQuery()));

		ini->getMolQuery()->setArraysAtoms(query->atoms.size());
		//cout << "Query"<<endl;
		ini->getMolQuery()->ObjectToArray(ini->getMolQuery()->getAtomsXYZ(),
		...
		ini->getMolQuery()->setTanimoto(ini->getMolQuery()->getAtomsXYZ(),
		...
		// ---- Se copia la molecula variable  para poder modificarla en caso de que sea necesario---
		//ini->setMoleculeTemp(new Molecule(ini->getMolVariable()));

		// -- Se establece el limite inferior de similitud. Si al division de los volumenes de ambas moleculas es inferior a
		// -- a este limite, entonces no se ejecuta el algoritmo. De esta forma se reduce el tiempo de busqueda
		if (indexArray[14] != -1) {
			ini->setLowLimit(atof(argv[indexArray[14]]));
		}
		// Comprobamos que el solapamiento entre ambas moleculas sean mayor que el indicado por parametro
		double minMol = min(ini->getMolVariable()->tanimoto,
				ini->getMolQuery()->tanimoto);
		double maxMol = max(ini->getMolVariable()->tanimoto,
				ini->getMolQuery()->tanimoto);
		if ((minMol / maxMol) < ini->getLowLimit()) {
			cerr
					<< "optipharm: -- Maximum posible score is lower than limit used: "
					<< ini->getLowLimit() << "\n";
			continue;
		}

		// --- Get Longest and shortest axis from the two molecules and configuring Upb y Lowb
		...
		if (ini == NULL || ini->Fail()) {
			...
			if (master == NULL || master->Fail()) {
				...
				for (long i = 1; i <= repcount; ++i) {
					...
					std::chrono::duration<double, std::ratio<1>> fp_ms = end
							- start;
