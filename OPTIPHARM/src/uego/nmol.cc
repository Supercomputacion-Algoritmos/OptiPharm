...
double NDimMolElement::UpdateValue() {
	...
	if (mV != 0) {
		...
		value = Value();
		x = normx; // -->Este valia cuando no se ponia el punto en la superficie de la esfera
	}
...
//  binom(2n,n) is approx. 2**(2n)/sqrt(pi*n)
	...
	double V[ SPEED_CONSTANTS] = {	// for r==1.0
			0.0, .25, M_2_PI / 3.0, 3.0 / 16.0, 4 * M_2_PI / 15.0, 10.0 / 64.0,
					8 * M_2_PI / 35.0, 35.0 / 256.0, 64 * M_2_PI / 315.0, 63.0
							/ 512.0, 128 * M_2_PI / 693.0, 231.0 / 2048.0, 512
							* M_2_PI / 3003.0, 858.0 / 8192.0 };
...
// -------------------------------------------------------------------------
/** Este es el mismo que el RandNew() pero que se le ha anadido por parametro una especie ya que no
 * interesa que todos los parametros se generen aleatoriamente.
 */
...
void NDimMolElement::Save(FILE* stream, bool best) {
	...
	for (unsigned i =0; i< INI.getMolQuery()->num_atoms;i++){
				...
				INI.getMolQuery()->atoms[i].y = INI.getMolQuery()->getAtomsXYZ()[iterador+1];
				INI.getMolQuery()->atoms[i].z = INI.getMolQuery()->getAtomsXYZ()[iterador+2];
			}
	for (unsigned i =0; i< INI.getMolVariable()->num_atoms;i++){
						iterador = i * 3;
						INI.getMolVariable()->atoms[i].x = INI.getMolVariable()->getAtomsXYZ()[iterador];
						INI.getMolVariable()->atoms[i].y = INI.getMolVariable()->getAtomsXYZ()[iterador+1];
						INI.getMolVariable()->atoms[i].z = INI.getMolVariable()->getAtomsXYZ()[iterador+2];
					}
//SAVE BEST RESULT
	double y;
	double xarray[dim];
...
// TRANSFORM THE MOLECULE AND SAVE IN MOL2 FILES FOR SHOWING IT
	...
	if (INI.getSaveSolutionMol2Files()) {
		...
		pathRotated = path;
		pathRotated.append(
				"MolR-" + INI.getMolQuery()->mol_name + "-"
						+ INI.getMolVariable()->mol_name + ".mol2");
		WriteMolecule::writeMol(pathRotated, moleculeRotated);

	}
	/*
	 * Here it is calculated the initial tanimoto value. We use the initial position of
	 * query molecule, variable molecule and the overlap between them.
	 */
	...
	INI.getMolVariable()->tanimoto,
	VolumeOverlap::overlapWEGA(
		INI.getMolQuery()->getAtomsXYZ(),
		INI.getMolQuery()->atoms.size(), INI.getMolQuery()->getWeightAtoms(),
		INI.getMolQuery()->getRadiusAtoms(), INI.getMolVariable()->getAtomsXYZ(),
		...
		INI.getSameVanDerWaalsRadius()));
			...
			string command = "./calc_et " + INI.getPathMolQuery() + " "
				+ INI.getPathMolVariable();

		string ls = GetStdoutFromCommand3(command);

		//string ls = GetStdoutFromCommand("python calc_et "+INI.getPathFixedMolecule()+" "+INI.getPathTempMolecule());
