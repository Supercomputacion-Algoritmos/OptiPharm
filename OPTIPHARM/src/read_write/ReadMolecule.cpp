...
void ReadMolecule::readMol(char *filePath, Molecule *molecule,
    ...
    if (!considerHydrogens) {
        ...
        for (int i = (molecule->num_bonds-1); i>-1;i--){
            if (atomsToDelete[molecule->bonds[i].origin_atom_id-1] || atomsToDelete[molecule->bonds[i].target_atom_id-1]){
                molecule->bonds.erase(
                        molecule->bonds.begin() + i);
            }
        }
        ...
        //Reasign id to bonds
        // To do so, for each id is compared the number of hydrogens removed until the id. Thus, that number of
        // hydrogens is remove from the original id.
        //cout << "Renaming"<<endl;
        for (unsigned  i = 0; i<molecule->num_bonds;i++){
            //cout << molecule->bonds[i].bond_id<< " " <<molecule->bonds[i].origin_atom_id <<" "<< molecule->bonds[i].target_atom_id<<endl;
            molecule->bonds[i].bond_id =  i +1;
            molecule->bonds[i].origin_atom_id -= idminusHydrogen[molecule->bonds[i].origin_atom_id-1];
            molecule->bonds[i].target_atom_id -= idminusHydrogen[molecule->bonds[i].target_atom_id-1];
            //cout << molecule->bonds[i].bond_id<< " " <<molecule->bonds[i].origin_atom_id << " " <<molecule->bonds[i].target_atom_id<<endl;
...
/*
 ...
 vector<Molecule> *conformations, bool considerHydrogens, bool sameVanDerWaalsRadius) {
 Molecule molecule;
